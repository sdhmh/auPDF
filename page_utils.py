...
from speech import convert_to_audio
from text_utils import clean_text


def parse_page(input_file: Path, output_file: Path, output_dir: Path | None = None) -> None:
    ...
    with open(input_file, "r", encoding="utf-8") as testfile:
        ...
        texts.lines = [convert_to_audio(line, output_directory=output_dir/"audio") for line in texts.lines]

    with open(output_dir/output_file, "w", encoding="utf-8") as jf:
        json.dump(texts.todict(), jf)

if __name__ == "__main__":
    parse_page(Path("test.txt"), Path("output.json"))
